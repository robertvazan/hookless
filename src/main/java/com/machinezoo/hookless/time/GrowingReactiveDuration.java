...
public class GrowingReactiveDuration extends ReactiveDuration implements Comparable<GrowingReactiveDuration> {
	...
	@Override public int compareTo(GrowingReactiveDuration other) {
	...
	@Override public GrowingReactiveDuration plus(Duration duration) {
	...
	@Override public GrowingReactiveDuration plus(long amount, TemporalUnit unit) {
		return plus(Duration.of(amount, unit));
	}
	@Override public GrowingReactiveDuration plusDays(long days) {
		return plus(Duration.ofDays(days));
	}
	@Override public GrowingReactiveDuration plusHours(long hours) {
		return plus(Duration.ofHours(hours));
	}
	@Override public GrowingReactiveDuration plusMinutes(long minutes) {
		return plus(Duration.ofMinutes(minutes));
	}
	@Override public GrowingReactiveDuration plusSeconds(long seconds) {
		return plus(Duration.ofSeconds(seconds));
	}
	@Override public GrowingReactiveDuration plusMillis(long millis) {
		return plus(Duration.ofMillis(millis));
	}
	@Override public GrowingReactiveDuration plusNanos(long nanos) {
		return plus(Duration.ofNanos(nanos));
	}
	@Override public GrowingReactiveDuration minus(Duration duration) {
		return plus(duration.negated());
	}
	public Duration minus(GrowingReactiveDuration other) {
		return Duration.between(zero, other.zero);
	}
	@Override public GrowingReactiveDuration minus(long amount, TemporalUnit unit) {
		return minus(Duration.of(amount, unit));
	}
	@Override public GrowingReactiveDuration minusDays(long days) {
		return minus(Duration.ofDays(days));
	}
	@Override public GrowingReactiveDuration minusHours(long hours) {
		return minus(Duration.ofHours(hours));
	}
	@Override public GrowingReactiveDuration minusMinutes(long minutes) {
		return minus(Duration.ofMinutes(minutes));
	}
	@Override public GrowingReactiveDuration minusSeconds(long seconds) {
		return minus(Duration.ofSeconds(seconds));
	}
	@Override public GrowingReactiveDuration minusMillis(long millis) {
		return minus(Duration.ofMillis(millis));
	}
	@Override public GrowingReactiveDuration minusNanos(long nanos) {
		return minus(Duration.ofNanos(nanos));
	}
	@Override public ShrinkingReactiveDuration negated() {
